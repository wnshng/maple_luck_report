...
class NexonMapleClient:
    ...
    def get_all_starforce_history_for_date(
        ...
        page = 0
        response = self.get_starforce_history_by_date(date=date, count=count)

        while True:
            page += 1
            records = response.get("starforce_history", [])
            if not isinstance(records, list):
                raise RuntimeError("스타포스 응답의 starforce_history가 list가 아닙니다.")
            ...
            self.last_debug_info.update(
                {
                    ...
                    "record_key_exists": "starforce_history" in response,
            ...
            if not next_cursor:
                break
            if page >= max_pages:
                raise RuntimeError("스타포스 페이지 수가 너무 많아 중단했습니다. 날짜 범위를 줄여주세요.")

            time.sleep(REQUEST_SLEEP_SECONDS)
            response = self.get_starforce_history_by_cursor(cursor=str(next_cursor), count=count)
